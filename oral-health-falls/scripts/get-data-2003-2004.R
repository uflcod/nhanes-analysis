...
smq_c <- nhanes('SMQ_C')
write_data(smq_c, 'SMQ_C_tobacco_use_questionnaire.tsv')

# medical conditions questionnaire
mcq_c <- nhanes('MCQ_C')
write_data(mcq_c, 'MCQ_C_medical_conditions_questionnaire.tsv')

# Rx questionnaire data
rx_c <- nhanes('RXQ_RX_C')
write_data(rx_c, 'RXQ_RX_C_rx_medications_questionnaire.tsv')
...
ohq_c <- nhanes('OHQ_C')
write_data(ohq_c, 'OHQ_C_oral_health_questionnaire.tsv')
